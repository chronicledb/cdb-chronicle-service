...
A gRPC service that acts as a write-ahead log gateway, enforcing strict per-chronicle transaction ordering before durably writing to Kafka.
...
Appends a transaction to the log for a given chronicle.

**Request**

| Field          | Type | Description                            |
|----------------|---|----------------------------------------|
| `chronicle_id` | string | The chronicle identifier       |
...
final AppendTxRequest request = AppendTxRequest.newBuilder()
        .setChronicleId("my-chronicle-id")
        .setSeqNum(1)
...
Each chronicle (`chronicle_id`) has its own independent sequence number starting at 1. The service rejects any transaction whose sequence number is not exactly `lastCommittedSeqNum + 1`. On `ABORTED`, the client should fetch the current committed sequence number and retry accordingly.

## Architecture

- **Stripe locking** — `chronicle_id`s are hashed across 1024 `ReentrantLock` stripes, allowing unrelated chronicles to commit in parallel while serializing concurrent writes to the same chronicle
